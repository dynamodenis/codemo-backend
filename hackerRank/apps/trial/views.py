...
from .serializers import MyQuizListSerializer, QuizDetailSerializer, QuizListSerializer, QuizResultSerializer, UsersAnswerSerializer,QuestionSerializer,QuizTakerSerializer
from rest_framework.generics import RetrieveUpdateAPIView
from rest_framework.permissions import AllowAny, IsAuthenticated
from rest_framework.views import APIView

class Hometrial(APIView):
    '''
	fetch question with the provided id from codewars
	'''
    def post(self, request):
        serializers = QuestionSerializer(data=request.data)
        response = requests.get('https://www.codewars.com/api/v1/code-challenges/53da3dbb4a5168369a0000fe?access_key='+settings.ACCESS_KEY)
        # transfor the response to json objects
        question=response.json()
        description=question["description"]
        
        trial=Quiz.objects.filter(pk=1) 
        id=trial.first().id
        
     
        new_question=Question(question=description, score=10)  
        new_question.quiz=id    
        new_question.save()
        return Response(new_question, status=status.HTTP_200_OK)
...
class StatisticsAPI(generics.RetrieveAPIView):
    ...
    def get(self, request,format=None):    
        ...
        serializers = QuizTakerSerializer(queryset, many=True)
